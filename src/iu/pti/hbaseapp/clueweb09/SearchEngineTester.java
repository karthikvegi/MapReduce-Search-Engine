	...
	public static void searchKeyword(String keyword) throws Exception {
        ...
        for (KeyValue kv : indexRow.list()) {
            ...
            // Step 2: get the URI of the page from clueWeb09DataTable
						Get pageUriIndex = new Get(Bytes.toBytes(pageDocId));
						Result pageRow = dataTable.get(pageUriIndex);
						pageUri = Bytes.toString(pageRow.getValue(Constants.CF_DETAILS_BYTES, Constants.QUAL_URI_BYTES));

            // Step 3: get the page rank value of this page from clueWeb09PageRankTable
            	Get pageRankIndex = new Get(Bytes.toBytes(pageDocId));
            	Result pageRankRow = prTable.get(pageRankIndex);
